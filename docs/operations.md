...
`forge-model-serve-probe` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for serve workflows, centered on incremental indexing, append-only fixtures, and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 4.
